...
def validate(draft_body: str, contract: dict, risk_triggers: list[str], scenario: str = "") -> dict:
    ...
    # Also strip any leading [REVIEW NEEDED] from body measurement
    _body_only = re.sub(r'^\[REVIEW NEEDED:[^\]]*\]\s*', '', _body_only, flags=re.IGNORECASE).strip()
    total_checks += 1
    if len(_body_only) < 60:
        issues.append(
